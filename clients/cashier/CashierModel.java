...
public class CashierModel extends Observable
{
  ...
  private enum State { process, reserved, checked }
   ...
   */
  public void doReserved() {
    String  theAction ="";
    int    amount  = 1;
    try {
      if ( theStock.exists( Reserved ) ) {
        Product pr = theStock.getDetails(Reserved);
        ...
        theState = (State.reserved);
      } else{
        theAction = "There Is nothing that has been requested to be reserved right now ";
      }
  ...
  public void doBuy()
  {
    String theAction = "";
    int    amount  = 1;                         //  & quantity
    if (theState == State.reserved){
      //System.out.println("Beep, Boop, The program reached this loop!");
      CashierView.ReservedUpdateReverse();
      Reserved = "";
      theState = State.checked;
      amount = 0; //change amount to 0 so that the program does
                  //drop the stock twice
    }
    ...
    try
    {
      ...
      {                                         //  with customer
        ...
        boolean stockBought =                   // Buy
          theStock.buyStock(
            theProduct.getProductNum(), amount);
